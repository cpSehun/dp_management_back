...
def _convert_orm_tags_to_list(prompt_orm: models.ImagePrompt) -> models.ImagePrompt:
    print(f"Before conversion - ID: {prompt_orm.id if hasattr(prompt_orm, 'id') else 'N/A'}, Tags: {prompt_orm.tags if hasattr(prompt_orm, 'tags') else 'N/A'}, Type: {type(prompt_orm.tags) if hasattr(prompt_orm, 'tags') else 'N/A'}") # 디버깅 추가
    if prompt_orm and hasattr(prompt_orm, 'tags') and isinstance(prompt_orm.tags, str):
        ...
        prompt_orm.tags = []
    elif prompt_orm and not hasattr(prompt_orm, 'tags'): # tags 속성 자체가 없는 경우 (이론상 발생 안 함)
        prompt_orm.tags = []
    print(f"After conversion - ID: {prompt_orm.id if hasattr(prompt_orm, 'id') else 'N/A'}, Tags: {prompt_orm.tags if hasattr(prompt_orm, 'tags') else 'N/A'}, Type: {type(prompt_orm.tags) if hasattr(prompt_orm, 'tags') else 'N/A'}") # 디버깅 추가
    return prompt_orm
...
def create_new_image_prompt(
    ...
    # Tags 처리: crud 함수에서 처리하도록 변경 (또는 여기서 처리 후 전달)
    # if isinstance(prompt_in.tags, list):
    #     prompt_in.tags = ",".join(prompt_in.tags)
    # elif prompt_in.tags is None:
    #     prompt_in.tags = None # 명시적으로 None 처리
    
    # 첫 번째 버전을 필수로 활성화 (Pydantic 스키마에서 처리하는 것이 더 좋을 수 있음)
    ...
    if not any(v.is_active for v in prompt_in.versions):
        # 첫 번째 버전을 활성으로 설정하거나, 클라이언트에서 활성 버전을 지정하도록 강제
        prompt_in.versions[0].is_active = True 
        # 또는 HTTPException(status_code=400, detail="One version must be active.")

    new_prompt = crud.create_image_prompt(db=db, prompt_in=prompt_in)
    # 태그를 문자열에서 리스트로 변환
...
async def update_single_image_prompt(
    ...
    updated_prompt_orm = crud.update_image_prompt(db, prompt_id=prompt_id, prompt_update_data=prompt_update_data)
...
async def delete_single_image_prompt(prompt_id: int, db: Session = Depends(get_db)):
...
# --- Prompt Version Routes (Rename to ImagePrompt Version Routes) ---
...
async def add_new_version_to_image_prompt(
    ...
    new_version = crud.create_image_prompt_version(db=db, prompt_id=prompt_id, version_data=version_data)
    return new_version
